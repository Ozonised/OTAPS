...
void GPIO_Init(void)
{
    ...
    /* All signal LED pin as output */
    GPIO_PortA.Pin = LED_SIGNAL_RED_GPIO_PIN | LED_SIGNAL_YELLOW1_GPIO_PIN | LED_SIGNAL_GREEN_GPIO_PIN | LED_SIGNAL_YELLOW2_GPIO_PIN;
    GPIO_PortA.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_PortA.Pull = GPIO_NOPULL;
    GPIO_PortA.Speed = GPIO_SPEED_FREQ_LOW;

    HAL_GPIO_WritePin(LED_SIGNAL_GPIO_PORT, LED_SIGNAL_RED_GPIO_PIN | LED_SIGNAL_YELLOW1_GPIO_PIN | LED_SIGNAL_GREEN_GPIO_PIN | LED_SIGNAL_YELLOW2_GPIO_PIN, GPIO_PIN_RESET);
    HAL_GPIO_Init(LED_SIGNAL_GPIO_PORT, &GPIO_PortA);

    /* Proximity Sensor pin as input with pull up and interrupt on falling edge */
    GPIO_PortB.Pin = PROXMITY_SNSR_GPIO_PIN;
    GPIO_PortB.Mode = GPIO_MODE_IT_FALLING;
    GPIO_PortB.Pull = GPIO_PULLUP;

    HAL_GPIO_Init(PROXMITY_SNSR_GPIO_PORT, &GPIO_PortB);
}

void NVIC_Init(void)
{
    __HAL_GPIO_EXTI_CLEAR_IT(PROXMITY_SNSR_GPIO_PIN);
...
{
    HAL_GPIO_EXTI_IRQHandler(PROXMITY_SNSR_GPIO_PIN);
}