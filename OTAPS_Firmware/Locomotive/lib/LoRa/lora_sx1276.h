...
// Set the expected payload length in bytes, it needs to be set in implicit header mode.
// Params:
// - `len` - length of the payload in bytes (greater than 0)
// See `RegPayloadLength` at section 6.4(LoRaTm Mode Register Map) page 114 of the datasheet.
void lora_set_payload_length(lora_sx1276 *lora, uint8_t len);
